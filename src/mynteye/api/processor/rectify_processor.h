...
class RectifyProcessor : public Processor {
 public:
  ...
  RectifyProcessor(
      ...
      std::shared_ptr<Extrinsics> extr, std::int32_t proc_period = 0);
 ...
 private:
  ...
  void stereoRectify(
      ...
      const cv::Mat *K2, const cv::Mat *D1, const cv::Mat *D2,
      cv::Size imageSize, const cv::Mat *matR, const cv::Mat *matT,
      cv::Mat *_R1, cv::Mat *_R2, cv::Mat *_P1, cv::Mat *_P2, double *T_mul_f,
      double *cx1_min_cx2, int flags = cv::CALIB_ZERO_DISPARITY,
      double alpha = -1, cv::Size newImgSize = cv::Size(0, 0));
