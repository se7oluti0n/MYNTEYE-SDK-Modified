...
class EquidistantCamera : public Camera {
 public:
  ...
  void liftProjective(
      const models::Vector2d &p, models::Vector3d &P) const;  // NOLINT
  // %output P

  // Projects 3D points to the image plane (Pi function)
  void spaceToPlane(
      const models::Vector3d &P, models::Vector2d &p) const;  // NOLINT
  // %output p
  ...
  cv::Mat initUndistortRectifyMap(
      cv::Mat &map1, cv::Mat &map2, float fx = -1.0f,
      float fy = -1.0f,  // NOLINT
...
typedef std::shared_ptr<const EquidistantCamera>
    EquidistantCameraConstPtr;  // NOLINT
